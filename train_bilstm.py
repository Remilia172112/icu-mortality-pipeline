"""
=============================================================
  Bi-LSTM Training — ICU Mortality Prediction
  GPU: RTX 5060 | RAM: 32GB
=============================================================

Cách chạy (Windows):
    python train_bilstm.py --data_dir E:\mimic4_processed

Cách chạy (Colab):
    !python train_bilstm.py --data_dir /content/drive/MyDrive/mimic4_processed

Output:
    checkpoints/
    ├── epoch_01.pt          ← checkpoint mỗi epoch
    ├── epoch_02.pt
...
# ══════════════════════════════════════════════════════════════════════
#  MODEL — Bi-LSTM with Missing Mask
# ══════════════════════════════════════════════════════════════════════
class BiLSTM_ICU(nn.Module):
    """
    Bidirectional LSTM cho ICU Mortality Prediction.

    Input:
        x: (batch, seq_len, n_features)          — vital signs + labs
        m: (batch, seq_len, n_features) optional  — missing mask (1=thật, 0=điền)

    Nếu use_mask=True, concat x và m → input dim = n_features * 2
    (Kỹ thuật Missing Indicator — ref: GRU-D paper, Che et al. 2018)

    Architecture:
        [Input] → [Bi-LSTM × n_layers] → [Dropout] → [FC → Sigmoid]
    """
    ...
    def forward(self, x, m=None):
        ...
        # Lấy output ở time step cuối (cả 2 hướng)
...
# ══════════════════════════════════════════════════════════════════════
def main():
    parser = argparse.ArgumentParser(description="Bi-LSTM ICU Mortality Training")
    parser.add_argument("--data_dir",    required=True, help="Thư mục chứa X/y/M .pt files")
    parser.add_argument("--ckpt_dir",    default="checkpoints", help="Lưu checkpoints")
    ...
    parser.add_argument("--batch_size",  type=int,   default=64,    help="Batch size (RTX 5060: 64-128)")
    parser.add_argument("--epochs",      type=int,   default=50,    help="Max epochs")
    parser.add_argument("--patience",    type=int,   default=10,    help="Early stopping patience")
    parser.add_argument("--use_mask",    type=int,   default=1,     help="1=dùng missing mask, 0=không")
    parser.add_argument("--use_sampler", type=int,   default=1,     help="1=WeightedSampler, 0=pos_weight")
    parser.add_argument("--pos_weight",  type=float, default=-1,    help="pos_weight cho BCELoss. -1 = tự tính từ data")

    args = parser.parse_args()
    log  = setup_logging(args.ckpt_dir)

    log.info("=" * 60)
    log.info("  Bi-LSTM ICU Mortality Prediction Training")
    log.info("=" * 60)

    # ── Device (hỗ trợ hybrid GPU mode) ─────────────────────────────
    if torch.cuda.is_available():
        device = torch.device("cuda")
        # Force chọn GPU rời (RTX) nếu có nhiều GPU (hybrid mode)
        if torch.cuda.device_count() > 1:
            # Chọn GPU có VRAM lớn nhất (thường là RTX, không phải iGPU)
        ...
        log.info("  Tip: Nếu có RTX nhưng hybrid mode, thử:")
        log.info("    1. Mở NVIDIA Control Panel → Manage 3D Settings")
        log.info("    2. Program Settings → thêm python.exe")
        log.info("    3. Preferred GPU → High-performance NVIDIA processor")
    ...
    log.info(f"  Model: BiLSTM (hidden={args.hidden_size}, layers={args.n_layers})")
    log.info(f"  Trainable parameters: {n_params:,}")

    # ── Loss, Optimizer, Scheduler ────────────────────────────────────
    # pos_weight: tự tính từ data nếu không truyền --pos_weight
    ...
    # PyTorch 2.4+ đã xóa tham số verbose
